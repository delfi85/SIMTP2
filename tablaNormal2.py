...
class NormWindow(QWidget):
    ...
    def calcular_chi_cuadrado(self, random_numbers, k_intervalos):
        ...
        frecuencias_esperadas = [len(random_numbers) * (
                    norm.cdf(limites_superiores[i], mu, sigma) - norm.cdf(limites_inferiores[i], mu, sigma)) for i in
                                 range(k_intervalos)]
    ...
    def mostrar_resultados_en_tabla(self, datos_tabla):
        ...
        self.histogram_window = HistogramDistWindow(self.random_numbers, self.k_intervalos)
    ...
    def mostrar_tabla_agrupada(self):
        ...
        # Cerrar la ventana del histograma si está abierta
